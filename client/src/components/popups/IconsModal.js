      ...
      Api.get("/getIcons?searchWord=" + queryString, {
        ...
        .then((res) => {
          if (!!res.data.icons && !!res.data.icons.length) {
            setIcons(
              res.data.icons.map((icon) => ({ url: icon, isLoading: true }))
            );
          } else {
            setIcons([]);
          }
          ...
          >
            ...
            >
              {isFetching && isSpinnerVisible && (
                <TemporarySpinner
                  destroySpinner={() => setIsSpinnerVisible(false)}
                />
              )}
            </div>
          </Grid>
                        ...
                        !!icons[4 * row + column] && (
                          ...
                          >
                            <img
                              ...
                              height={isMobileOnly ? "50px" : "68px"}
                              style={{
                                display: icons[4 * row + column].isLoading
                                  ? "none"
                                  : "block",
